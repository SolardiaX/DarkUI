...
addon.Hooks["datatext"] = function(a)
    ...
    master:HookScript("OnClick", function(self)
        for path, w in pairs(a.widgets) do
            if path:find("^datatext%.") and path ~= "datatext.enable" then w:SetEnabled(self:GetChecked()) end
        end
    end)
    for path, w in pairs(a.widgets) do
        if path:find("^datatext%.") and path ~= "datatext.enable" then w:SetEnabled(checked) end
    end
end
