...
- [ ] **冒险指南** `EncounterJournal.lua` — `Shift-J` — `encounterjournal`:难度下拉、首领列表、战利品图标品质边、模型框
- [ ] **成就** `Achievement.lua` — `Y` — `achievement`:分类侧栏、成就卡片、图标
- [ ] **好友列表** `Friends.lua` — `O` — `friends`:标签、列表项、添加好友输入框
