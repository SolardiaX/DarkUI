...
------------------------------------------------------------------------
-- Friends frame (friends list / who / RAF / quick join)
-- Ported from ElvUI Mainline/Skins/Friends.lua (v15.15, 2026-06)
-- Manual divergences: the few multi-arg CreateBackdrop(..., allPoints) calls are
-- rewritten to CreateBackdrop(template) + backdrop:SetAllPoints() (our atom uses a
-- different positional signature; see Skins/SYNC.md).
------------------------------------------------------------------------
...
local function BattleNetFrame_OnClick() _G.FriendsFrameBattlenetFrame.BroadcastFrame:ToggleFrame() end
...
local function RAFRewards()
    ...
    if claiming and claiming.NextRewardButton then claiming.NextRewardButton.Icon:SetDesaturation(0) end
    ...
    for tab in rewardsFrame.rewardTabPool:EnumerateActive() do
        if not tab.__styled then
            ...
            if relativeTo and relativeTo == rewardsFrame then tab:NudgePoint(2, 0) end
...
local function ReskinFriendButton(button)
    ...
    if summon then
        summon:CreateBackdrop("Transparent")
        summon.backdrop:SetAllPoints()
        summon:Size(24)
    ...
    invite:CreateBackdrop("Transparent")
    invite.backdrop:SetAllPoints()
    invite.NormalTexture:SetAlpha(0)
    invite.PushedTexture:SetAlpha(0)
    invite.DisabledTexture:SetAlpha(0)
    invite.HighlightTexture:SetColorTexture(1, 1, 1, 0.25)
...
function S:FriendsFrame()
    ...
    for i = 1, 17 do
        ...
        if level then level:Width(level:GetWidth() + 5) end
    end
