...
------------------------------------------------------------------------
-- Transmogrification
-- Ported from ElvUI Mainline/Skins/Transmog.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
local function SkinSituationsDropdowns()
    ...
    if Situations then
        for _, child in next, { Situations:GetChildren() } do
            if child.Dropdown and not child.Dropdown.__styled then
                S:HandleDropDownBox(child.Dropdown, 300)

                child.Dropdown.__styled = true
            end
        end
    end
end
...
function S:Blizzard_Transmog()
    if not C.skins.transmogrify then return end
    ...
    if HelpPlateButton then
        ...
        if HelpPlateButton.Ring then HelpPlateButton.Ring:Hide() end
    end
    ...
    if OutfitCollection then
        if C.skins.parchment_remover then
            ...
            if not OutfitCollection.backdrop then OutfitCollection:CreateBackdrop("Transparent") end
        end

        if OutfitCollection.GradientTop then OutfitCollection.GradientTop:Hide() end

        if OutfitCollection.GradientBottom then OutfitCollection.GradientBottom:Hide() end
    ...
    if CharacterPreview then
        if C.skins.parchment_remover then
            ...
            if not CharacterPreview.backdrop then CharacterPreview:CreateBackdrop("Transparent") end
        end
    ...
    if WardrobeCollection.TabContent then
        if C.skins.parchment_remover then
            ...
            if not WardrobeCollection.backdrop then WardrobeCollection:CreateBackdrop("Transparent") end
        end
        ...
        if SituationsFrame then
            if C.skins.parchment_remover then
                ...
                if not SituationsFrame.Situations.backdrop then SituationsFrame.Situations:CreateBackdrop("Transparent") end
            end
