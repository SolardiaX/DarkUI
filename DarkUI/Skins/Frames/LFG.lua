...
------------------------------------------------------------------------
-- Looking for group / Mythic+ / PVP queue
-- Ported from ElvUI Mainline/Skins/LFG.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
local function HandleAffixIcons(child)
    ...
    if MapID then
        ...
        if Name and PowerLevel then child.DungeonName:SetText(Name .. " |cffffffff-|r (" .. PowerLevel .. ")") end
    ...
    for _, frame in ipairs(list) do
        if frame.Border then frame.Border:SetTexture() end

        if frame.Portrait then frame.Portrait:SetTexture() end
...
local function LFDCheckboxMini_SetTexture(region, texture)
    if texture ~= E.Media.Textures.Melli then region:SetTexture(E.Media.Textures.Melli) end
end
...
local function LFDQueueFrameSpecificUpdate(frame) frame:ForEachFrame(LFDQueueFrameSpecificUpdateChild) end
...
function S:LookingForGroupFrames()
    ...
    for _, roleButton in pairs({
        ...
        if checkButton:GetScale() ~= 1 then checkButton:SetScale(1) end
    ...
    hooksecurefunc("SetCheckButtonIsRadio", function(button)
        if not button.__styled then S:HandleCheckBox(button) end
    end)
    ...
    hooksecurefunc("LFGListApplicationDialog_UpdateRoles", function(dialog) -- Copy from Blizzard, we just fix position
        ...
        local avail1, avail2
        if availTank then avail1 = dialog.TankButton end
        if availHealer then
            if avail1 then
    ...
    hooksecurefunc("LFG_EnableRoleButton", function(button) button.checkButton:SetAlpha(1) end)
    ...
    do
        ...
        while button do
            ...
            if button.bg then button.bg:Kill() end
    ...
    if NothingAvailable then NothingAvailable.Inset:StripTextures() end
    ...
    hooksecurefunc("LFGListApplicationViewer_UpdateApplicant", function(button)
        if not button.DeclineButton.__template then S:HandleButton(button.DeclineButton, nil, true) end
        if not button.InviteButton.__template then S:HandleButton(button.InviteButton) end
        if not button.InviteButtonSmall.__template then S:HandleButton(button.InviteButtonSmall) end
    end)

    hooksecurefunc("LFGListSearchEntry_Update", function(button)
        if not button.CancelButton.__template then S:HandleButton(button.CancelButton, nil, true) end
    end)
...
function S:Blizzard_ChallengesUI()
    ...
    KeyStoneFrame.KeystoneSlot:HookScript("OnEvent", function(frame, event, itemID)
        if event == "CHALLENGE_MODE_KEYSTONE_SLOTTED" and frame.Texture then
            ...
            if texture then frame.Texture:SetTexture(texture) end
        end
    end)
    ...
    hooksecurefunc(ChallengesFrame, "Update", function(frame)
        for _, child in ipairs(frame.DungeonIcons) do
            if not child.__template then
                child:GetRegions():SetAlpha(0)
                child:SetTemplate()
                child.Icon:SetInside()
                S:HandleIcon(child.Icon)
            end
            -- ElvUI reordered child.Center (its template's center texture); DarkUI's
            -- BackdropTemplate has none, so nothing to do here.
        end
    end)

    hooksecurefunc(_G.ChallengesFrameWeeklyInfoMixin, "SetUp", function(info)
        if C_MythicPlus_GetCurrentAffixes() then HandleAffixIcons(info.Child) end
    end)
