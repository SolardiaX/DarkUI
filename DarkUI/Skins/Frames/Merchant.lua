...
local function SetItemButtonScale(button, scale)
    if button.Count and scale ~= 1 then button.Count:SetScale(1) end
end
...
function S:MerchantFrame()
    ...
    for i = 1, _G.BUYBACK_ITEMS_PER_PAGE do
        ...
        if iconOverlay then iconOverlay:SetInside(button, 1, 1) end
    end
    ...
    S:HandleItemButton(_G.MerchantBuyBackItemItemButton)

    local UndoFrame = _G.UndoFrame
    if UndoFrame then UndoFrame.Arrow:Point("CENTER", _G.MerchantBuyBackItemItemButton) end
    ...
    if _G.MerchantSellAllJunkButton then HandleIconButton(_G.MerchantSellAllJunkButton, 0.34, 0.1, 0.34, 0.535, 0.535, 0.1, 0.535, 0.535) end
