...
------------------------------------------------------------------------
-- Character frame / paper doll
-- Ported from ElvUI Mainline/Skins/Character.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
local function UpdateTokenSkins(frame) frame:ForEachFrame(UpdateTokenSkinsChild) end
...
local function EquipmentManagerPane_UpdateChild(child)
    if child.icon and not child.__styled then
        ...
        child.SelectedBar:SetColorTexture(0.8, 0.8, 0.8, 0.25)
...
local function EquipmentManagerPane_Update(frame) frame:ForEachFrame(EquipmentManagerPane_UpdateChild) end
...
local function TitleManagerPane_Update(frame) frame:ForEachFrame(TitleManagerPane_UpdateChild) end
...
local function UpdateCharacterInset(name) _G.CharacterFrameInset.backdrop:SetShown(showInsetBackdrop[name]) end

local function UpdateAzeriteItem(item)
    if not item.__styled then
        item.__styled = true
...
local function TabTextureCoords(tex, x1)
    if x1 ~= 0.16001 then tex:SetTexCoord(0.16001, 0.86, 0.16, 0.86) end
end
...
local function UpdateFactionSkins(frame) frame:ForEachFrame(UpdateFactionSkinsChild) end

local function PaperDollUpdateStats()
    for frame in _G.CharacterStatsPane.statsFramePool:EnumerateActive() do
        if not frame.leftGrad then ColorizeStatPane(frame) end
...
local function BackdropDesaturated(background, value)
    if value and background.ignoreDesaturated then background:SetDesaturated(false) end
end
...
local function UpdateCurrencyTransferLogLines(frame) frame:ForEachFrame(UpdateCurrencyTransferLogLine) end
...
function S:Blizzard_UIPanels_Game()
    ...
    if TokenPopupClose then S:HandleCloseButton(TokenPopupClose) end
