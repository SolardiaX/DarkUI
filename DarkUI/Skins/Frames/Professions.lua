...
------------------------------------------------------------------------
-- Professions
-- Ported from ElvUI Mainline/Skins/Professions.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
local function HandleItemFlyoutContents(child)
    ...
    if child.ScrollBar then S:HandleTrimScrollBar(child.ScrollBar) end
...
local function HandleOutputButtons(frame) frame:ForEachFrame(HandleOutputButton) end
...
local function HandleSchematicInit(form)
    ...
    if form.salvageSlot then ReskinSlotButton(form.salvageSlot.Button) end

    if form.enchantSlot then ReskinSlotButton(form.enchantSlot.Button) end
end
...
local function HandleSchematicForm(form, noParchment)
    ...
    if form.Background then form.Background:SetInside(form.backdrop) end

    if noParchment or C.skins.parchment_remover then
        ...
        if form.MinimalBackground then form.MinimalBackground:SetAlpha(0.6) end
    end
...
local function HandleOrderView(frame)
    ...
    if frame.SetOverrideCastBarActive ~= E.noop then frame.SetOverrideCastBarActive = E.noop end
...
function S:Blizzard_Professions()
    ...
    if CraftingPage.SetOverrideCastBarActive ~= E.noop then CraftingPage.SetOverrideCastBarActive = E.noop end
