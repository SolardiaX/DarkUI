...
------------------------------------------------------------------------
-- Auction house
-- Ported from ElvUI Mainline/Skins/AuctionHouse.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
local function HandleSummaryIcon(child)
    if child.Icon then
        if not child.__styled then
            ...
            if child.IconBorder then child.IconBorder:Kill() end
...
local function HandleSummaryIcons(frame) frame:ForEachFrame(HandleSummaryIcon) end
...
local function HandleHeaders(frame)
    ...
    for i, header in next, { frame.HeaderContainer:GetChildren() } do
        ...
        if header.backdrop then header.backdrop:Point("BOTTOMRIGHT", i < maxHeaders and -5 or 0, -2) end
    end
...
local function HandleSellFrame(frame)
    ...
    if ItemButton.IconBorder then S:HandleIconBorder(ItemButton.IconBorder, ItemButton.Icon.backdrop) end
...
local function HandleSellList(frame, hasHeader, fitScrollBar)
    ...
    if frame.RefreshFrame then HandleAuctionButtons(frame.RefreshFrame.RefreshButton) end
...
local function HandleTabs(arg1)
    ...
    for index, tab in next, frame.Tabs do
        ...
        if blizzTab and not tab.backdrop then S:HandleTab(tab) end
...
local function LoadSkin()
    ...
    S:HandleIcon(ItemButton.Icon, true)
    ItemButton.Icon.backdrop:SetBackdropBorderColor(0, 0.8, 1)
    ItemButton:GetHighlightTexture():Hide()
    ItemButton.CircleMask:Hide()
    ...
    progressBar:CreateBackdrop() -- ElvUI passed allPoints
    progressBar.backdrop:SetAllPoints()
