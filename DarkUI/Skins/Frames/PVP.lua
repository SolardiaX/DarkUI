...
------------------------------------------------------------------------
-- PVP UI
-- Ported from ElvUI Mainline/Skins/PVP.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
local function SpecificScrollUpdate(frame) frame:ForEachFrame(SpecificScrollUpdateChild) end
...
function S:Blizzard_PVPUI()
    ...
    for i = 1, 4 do
        ...
        if bu then
            if bu.Ring then bu.Ring:Kill() end

            if bu.Background then bu.Background:Kill() end
    ...
    if PlunderstormPanel then S:HandleButton(PlunderstormPanel.PlunderstoreButton) end
        ...
        if currencyRewards then
            for _, reward in next, currencyRewards do
                ...
                if info and info.quality == ITEMQUALITY_ARTIFACT then
                    _, rewardTexture, _, rewardQuaility =
                        CurrencyContainerUtil_GetCurrencyContainerInfo(reward.id, reward.quantity, info.name, info.iconFileID, info.quality)
                end
            end
        end
    ...
    if C.skins.tooltip then TT:SetStyle(_G.ConquestTooltip) end
...
function S:PVPReadyDialog()
    ...
    hooksecurefunc("PVPReadyDialog_Display", function(dialog, _, _, isRated, queueType)
        ...
        if queueType == "BATTLEGROUND" and not isRated then dialog.background:SetTexCoord(0, 1, 0.01, 1) end
    end)
end
