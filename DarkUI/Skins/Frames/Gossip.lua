...
------------------------------------------------------------------------
-- Gossip / ItemText frame
-- Ported from ElvUI Mainline/Skins/Gossip.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
local function Gossip_SetTextColor(text, r, g, b)
    if r ~= 1 or g ~= 1 or b ~= 1 then text:SetTextColor(1, 1, 1) end
end

local function Gossip_ReplaceColor(color) return "|cFF" .. (GossipTextColors[color] or color) end
...
local function ItemTextPage_SetTextColor(pageText, headerType, r, g, b)
    if r ~= 1 or g ~= 1 or b ~= 1 then pageText:SetTextColor(headerType, 1, 1, 1) end
end
...
local function GreetingPanel_Update(frame) frame:ForEachFrame(GreetingPanel_UpdateChild) end

local function GossipFrame_SetAtlas(frame) frame:Height(frame:GetHeight() - 2) end
