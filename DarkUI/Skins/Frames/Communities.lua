...
------------------------------------------------------------------------
-- Communities frame (guild & community panels, club finder)
-- Ported from ElvUI Mainline/Skins/Communities.lua (v15.15, 2026-06)
-- Manual divergences:
--   * ChatTab anchor uses a fixed 0 offset (DarkUI runs pixel mode; ElvUI used
--     `E.PixelMode and 0 or E.Border + E.Spacing`).
--   * The guild faction StatusBar is given its texture directly; ElvUI's
--     E:RegisterStatusBar (live media hot-swap) has no DarkUI equivalent and is
--     dropped — the bar still renders with our texture.
------------------------------------------------------------------------
...
local function HandleCommunitiesButton(button)
    ...
    if not button.backdrop then button:CreateBackdrop("Transparent") end
...
local function HandleCommunityCards(frame) frame:ForEachFrame(HandleCommunityCardsChild) end
...
local function HandleRewardButtons(frame) frame:ForEachFrame(HandleRewardButton) end

local function CommunitiesListScrollUpdate(frame) frame:ForEachFrame(HandleCommunitiesButton) end
...
function S:Blizzard_Communities()
    ...
    CommunitiesFrame.ChatTab:Point("TOPLEFT", nil, "TOPRIGHT", 0, -36)
    ...
    -- ElvUI E:RegisterStatusBar(StatusBar) dropped: no live media hot-swap in DarkUI.
    ...
    hooksecurefunc("GuildNewsButton_SetNews", function(button, news_id)
        ...
        if newsInfo and button.header and button.header:IsShown() then button.header:SetAlpha(0) end
    end)
