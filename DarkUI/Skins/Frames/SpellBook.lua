...
------------------------------------------------------------------------
-- Professions book
-- Ported from ElvUI Mainline/Skins/SpellBook.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
local function ProfessionButtonUpdate(button)
    ...
    if C.skins.parchment_remover then
        if button.spellString then button.spellString:SetTextColor(1, 1, 1) end
        if button.subSpellString then button.subSpellString:SetTextColor(1, 1, 1) end
...
local function HandleSkillButton(button)
    ...
    if button.cooldown then E:RegisterCooldown(button.cooldown) end
...
function S:Blizzard_ProfessionsBook()
    if not (C.skins.enable and C.skins.spellbook) then return end
    ...
    for _, button in next, { _G.PrimaryProfession1, _G.PrimaryProfession2, _G.SecondaryProfession1, _G.SecondaryProfession2, _G.SecondaryProfession3 } do
        ...
        if button.unlearn then button.unlearn:Point("RIGHT", button.statusBar, "LEFT", -18, -5) end
        ...
        if button.icon then
            ...
            -- ElvUI: CreateBackdrop(...allPoints) + backdrop.Center draw-layer + a
            -- callbackBackdropColor closure. DarkUI's backdrop is a BackdropTemplate
            -- (no .Center sub-texture, no color callback), so just create it,
            -- reposition around the icon, and set the fill color directly.
    ...
    for i = 1, 2 do
        ...
        if button.iconTexture then S:HandleIcon(button.iconTexture, true) end
    end
