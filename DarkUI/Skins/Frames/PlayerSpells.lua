...
------------------------------------------------------------------------
-- Player spells (spellbook / talents)
-- Ported from ElvUI Mainline/Skins/PlayerSpells.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
local function HandleHeroTalents(frame)
    ...
    for specFrame in frame.SpecContentFramePool:EnumerateActive() do
        if specFrame and not specFrame.__styled then
...
function S:Blizzard_PlayerSpells()
    ...
    if CreateDialog then HandleTalentFrameDialog(CreateDialog) end
    ...
    if SpellBookFrame then
        ...
        if SpellBookFrame.TopBar then SpellBookFrame.TopBar:Hide() end

        if SpellBookFrame.BookCornerFlipbook then SpellBookFrame.BookCornerFlipbook:Hide() end
        ...
        if PagedSpellsFrame then
            if PagedSpellsFrame.View1 then PagedSpellsFrame.View1:DisableDrawLayer("OVERLAY") end
        ...
        if RotationButton then
            ...
            if RotationButton.Border then RotationButton.Border:Hide() end
