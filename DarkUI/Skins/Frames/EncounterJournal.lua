...
------------------------------------------------------------------------
-- Encounter journal (dungeon / raid guide)
-- Ported from ElvUI Mainline/Skins/EncounterJournal.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
do -- this prevents a taint trying to force a color lock by setting it to E.noop
    ...
    local function LockValue(button, r, g, b)
        ...
        if r ~= rr or gg ~= g or b ~= bb then button:SetTextColor(rr, gg, bb) end
    end

    local function LockWhite(button, r, g, b)
        if r ~= 1 or g ~= 1 or b ~= 1 then button:SetTextColor(1, 1, 1) end
    end
...
local function ItemSetElements(set)
    local parchment = C.skins.parchment_remover
    if parchment and not set.backdrop then set:CreateBackdrop() end

    if parchment and set.Background then set.Background:Hide() end

    if set.ItemButtons then
        for _, button in next, set.ItemButtons do
            local icon = button.Icon
            if icon and not icon.backdrop then S:HandleIcon(icon, true) end

            local border = button.Border
            if border and not border.__styled then
                border:SetAlpha(0)

                ItemSetsItemBorder(border, border:GetAtlas()) -- handle first one
                hooksecurefunc(border, "SetAtlas", ItemSetsItemBorder)

                border.__styled = true
            end
        end
    end
end

local function HandleItemSetsElements(frame) frame:ForEachFrame(ItemSetElements) end
...
local function InstanceSelectScrollUpdate(frame) frame:ForEachFrame(InstanceSelectScrollUpdateChild) end
...
local function BossesScrollUpdate(frame) frame:ForEachFrame(BossesScrollUpdateChild) end
...
local function LootContainerUpdate(frame) frame:ForEachFrame(LootContainerUpdateChild) end

local function JourneysListUpdateChild(child)
    -- This check is to avoid separators and headers from being skinned
    if (child.RenownCardFactionName or child.JourneyCardName) and not child.__styled then
        -- ToDo: Doesn't look great
        -- child:StripTextures()
        -- child:SetTemplate()

        local watchedFactionToggle = child.WatchedFactionToggleFrame
        if watchedFactionToggle and watchedFactionToggle.WatchFactionCheckbox then S:HandleCheckBox(watchedFactionToggle.WatchFactionCheckbox) end

        child.__styled = true
    end
end

local function JourneysListUpdate(frame) frame:ForEachFrame(JourneysListUpdateChild) end
...
local function CreateCollapseButton(frame, button, collapse)
    ...
    if not btn.button then btn.button = button end

    if not btn.collapse then btn.collapse = collapse end
...
function S:Blizzard_EncounterJournal()
    ...
    if E.global.general.disableTutorialButtons then EJ.MonthlyActivitiesFrame.HelpButton:Kill() end
    ...
    if JourneysList then hooksecurefunc(JourneysList, "Update", JourneysListUpdate) end

    -- Monthly Activities
    local MonthlyActivities = _G.EncounterJournalMonthlyActivitiesFrame
    if MonthlyActivities then hooksecurefunc(MonthlyActivities.ScrollBox, "Update", HandleCollapseButtons) end
    ...
    if C.skins.parchment_remover then
        ...
        hooksecurefunc("EJSuggestFrame_RefreshDisplay", function()
            for i, data in ipairs(suggestFrame.suggestions) do
                ...
                if sugg then
                    if not sugg.icon.backdrop then sugg.icon:CreateBackdrop() end
        ...
        hooksecurefunc("EJSuggestFrame_UpdateRewards", function(sugg)
            ...
            if rewardData then
                if not sugg.reward.icon.backdrop then
                    sugg.reward.icon:CreateBackdrop() -- ElvUI passed frameLevel 3; default layering is fine here
                end
    ...
    if C.skins.parchment_remover then
        ...
        if parchment then parchment:Kill() end
    end
    ...
    if Contents then
        if C.skins.parchment_remover then
            if not Contents.backdrop then Contents:CreateBackdrop() end
        ...
        -- ElvUI pushed backdrop.Center to a low draw layer; DarkUI's BackdropTemplate
        -- has no .Center sub-texture, so nothing to reorder here.
    end
