...
------------------------------------------------------------------------
-- Click binding UI
-- Ported from ElvUI Mainline/Skins/Binding.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
local function UpdateNewGlow(self) self.backdrop:SetBackdropBorderColor(0, self.NewOutline:IsShown() and 0.8 or 0, 0) end

local function HandleScrollUpdateChild(child)
    ...
    if icon and not icon.__styled then
        ...
        -- ElvUI: CreateBackdrop(nil, nil, nil, true, nil, nil, nil, true) — forcePixelMode + allPoints
        ...
        child.FrameHighlight:SetColorTexture(1, 1, 1, 0.20)
...
local function HandleScrollUpdate(frame) frame:ForEachFrame(HandleScrollUpdateChild) end

local function UpdateButtonColor(button, isSelected)
    if isSelected then
        button.Portrait.backdrop:SetBackdropBorderColor(1, 0.8, 0)
    else
...
function S:Blizzard_ClickBindingUI()
    ...
    hooksecurefunc(frame.ScrollBox, "Update", HandleScrollUpdate)
