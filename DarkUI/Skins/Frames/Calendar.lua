...
------------------------------------------------------------------------
-- Calendar
-- Ported from ElvUI Mainline/Skins/Calendar.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
local function SkinContainer(frame, container)
    ...
    if child and not child.backdrop then child:CreateBackdrop("Transparent") end
end

local function StripClassTextures(button, classFile) button:SetTexCoord(E:GetClassCoords(classFile, true)) end

local function HandleEventIcon(icon)
    icon:Size(54)
    icon:ClearAllPoints()
    icon:Point("TOPLEFT", _G.CalendarViewEventFrame.HeaderFrame, "TOPLEFT", 15, -20)
    icon:CreateBackdrop() -- ElvUI passed allPoints; default SetOutside(1) reads the same on an icon
    icon:SetTexCoords()
