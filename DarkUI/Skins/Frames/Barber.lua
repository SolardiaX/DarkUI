...
------------------------------------------------------------------------
-- Barber shop
-- Ported from ElvUI Mainline/Skins/Barber.lua (v15.15, 2026-06)
------------------------------------------------------------------------
...
local function SetSelectedCategory(list)
    if list.selectionPopoutPool then
        for frame in list.selectionPopoutPool:EnumerateActive() do
            if not frame.__styled then
                ...
                if frame.Label then frame.Label:FontTemplate() end
    ...
    if pool then
        for frame in pool:EnumerateActive() do
            if not frame.__styled then
                if frame.Button then S:HandleCheckBox(frame.Button) end
...
function S:Blizzard_CharacterCustomize()
    if not (C.skins.enable and C.skins.barber) then return end -- yes, it belongs also to tbe BarberUI
