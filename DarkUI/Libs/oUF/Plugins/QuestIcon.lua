...
local function GetQuests(unitID)
    ...
    if not canaccessvalue(unitID) or IsInInstance() then return end
            ...
            if count then
                ...
                if activeID then
                    ...
                    for j = 1, GetNumQuestLeaderBoards(index) or 0 do
                        local objText, objectiveType, finished = GetQuestObjectiveInfo(activeID, j, false)
                        if objText and not finished and (objectiveType == "item" or objectiveType == "object") then
                            type = "LOOT_ITEM"
                        end
                    end
                end
