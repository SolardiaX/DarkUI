...
function BagButton:UpdateButton()
    ...
    if self.bagId > maxBagSlots then
        local numSlots = GetNumBankSlots and GetNumBankSlots() or 6
        if self.bagId - maxBagSlots <= numSlots then
...
function BagButton:OnClick(btn)
    ...
    if self.notBought then
        ...
        BankFrame.nextSlotCost = GetBankSlotCost and GetBankSlotCost(numSlots) or 0
