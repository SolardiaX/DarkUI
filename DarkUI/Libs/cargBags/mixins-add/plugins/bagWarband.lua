...
local function UpdateTooltip(self, id)
    if not AccountBankPanel or not AccountBankPanel.purchasedBankTabData then return end
...
function BagWarbandButton:Create(bagID)
    ...
    local size = C.bags.itemSlotSize or 32
    ...
    button.__backdrop:SetBackdropColor(.3, .3, .3, .3)
...
cargBags:RegisterPlugin("BagWarband", function(self, bags)
    ...
    if AccountBankPanel then
        hooksecurefunc(AccountBankPanel, "RefreshBankTabs", function()
            RefreshBar(bar)
        end)
    end
