...
local numf
if cfg.damage or cfg.healing then numf = 4 else numf = 3 end
...
local function LimitLines()
    for i = 1, #ct.frames do
        f = ct.frames[i]
        if i == 4 and cfg.icons then
            f:SetMaxLines(math.floor(f:GetHeight() / (cfg.icon_size * 1.5)))
        else
            f:SetMaxLines(math.floor(f:GetHeight() / cfg.font.combat_text_font_size - 1))
        end
    end
end
...
local function OnEvent(_, event, subevent, powerType)
    if event == "COMBAT_TEXT_UPDATE" then
        ...
        if not CVarCallbackRegistry:GetCVarValueBool("enableFloatingCombatText") then
            ...
            if subevent == "DAMAGE" then
                if cfg.short_numbers == true then
            ...
            elseif subevent == "DAMAGE_CRIT" then
                ...
                xCT1:AddMessage("|cffFF0000" .. cfg.crit_prefix .. "|r" .. "-" .. arg2 .. "|cffFF0000" .. cfg.crit_postfix .. "|r", 1, 0.1, 0.1)
            ...
            elseif subevent == "SPELL_DAMAGE_CRIT" then
                ...
                xCT1:AddMessage("|cffFF0000" .. cfg.crit_prefix .. "|r" .. "-" .. arg2 .. "|cffFF0000" .. cfg.crit_postfix .. "|r", 1, 0.3, 0.5)
            elseif subevent == "HEAL" then
                if arg3 >= cfg.heal_treshold then
                    if cfg.short_numbers == true then
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        ...
        if cfg.damage or cfg.healing then
...
-- Change damage font
if cfg.damage_style then DAMAGE_TEXT_FONT = cfg.font.combat_text_font end

-- Hide/Show blizzard combat text
if cfg.blizz_head_numbers ~= true then
...
for i = 1, numf do
    ...
    f:SetFont(cfg.font.combat_text_font, cfg.font.combat_text_font_size, cfg.font.combat_text_font_style)
    f:SetShadowColor(0, 0, 0, cfg.font.combat_text_font_shadow and 1 or 0)
    f:SetShadowOffset(cfg.font.combat_text_font_shadow and 1 or 0, cfg.font.combat_text_font_shadow and -1 or 0)
    f:SetTimeVisible(cfg.time_visible)
    f:SetMaxLines(cfg.max_lines)
    f:SetSpacing(1)
    f:SetWidth(128)
    f:SetHeight(112)
    ...
    elseif i == 3 then
        ...
        if cfg.font.combat_text_font_size == "auto" then
            if cfg.icons then
                f:SetFont(a, cfg.icon_size / 2, c)
            end
        elseif type(cfg.font.combat_text_font_size) == "number" then
            f:SetFont(a, cfg.font.combat_text_font_size, c)
        end
    end
    ct.frames[i] = f
end
...
if cfg.dk_runes and E.class == "DEATHKNIGHT" then
...
local StartConfigmode = function()
    if not InCombatLockdown() then
        for i = 1, #ct.frames do
            ...
            f.fs:SetFont(cfg.font.combat_text_font, cfg.font.combat_text_font_size, cfg.font.combat_text_font_style)
            ...
            if not cfg.scrollable then
                if i == 4 and cfg.icons then
                    f:SetScript("OnSizeChanged", function(self)
                        self:SetMaxLines(math.floor(self:GetHeight() / (cfg.combattext.icon_size * 1.5)))
                        self:Clear()
                    end)
                else
                    f:SetScript("OnSizeChanged", function(self)
                        self:SetMaxLines(math.floor(self:GetHeight() / cfg.font.combat_text_font_size - 1))
                        self:Clear()
                    end)
                end
            end
        ...
        print("|cffffff00" .. L.COMBATTEXT_UNLOCKED .. "|r")
    else
...
local function EndConfigmode()
    ...
    ct.locked = true
    print("|cffffff00" .. L.COMBATTEXT_UNSAVED .. "|r")
end
        ...
        ct.frames[i]:SetScript("OnUpdate", function(_, elapsed)
            ...
            if TimeSinceLastUpdate > UpdateInterval then
                ...
                elseif i == 4 then
                    ...
                    if icon then
                        msg = msg .. " \124T" .. icon .. ":" .. cfg.icon_size .. ":" .. cfg.icon_size .. ":0:0:64:64:5:59:5:59\124t"
                        if cfg.damage_color then
                            color = ct.dmgcolor[ct.dmindex[random(#ct.dmindex)]]
                        else
                            color = { 1, 1, 0 }
                        end
                    elseif cfg.damage_color and not cfg.icons then
                        color = ct.dmgcolor[ct.dmindex[random(#ct.dmindex)]]
                    elseif not cfg.damage_color then
...
SlashCmdList.XCT = function(input)
    ...
    if input == "unlock" then
        if ct.locked then
            ...
            print("|cffffff00" .. L.COMBATTEXT_ALREADY_UNLOCKED .. "|r")
        end
    elseif input == "lock" then
        if ct.locked then
            print("|cffffff00" .. L.COMBATTEXT_ALREADY_LOCKED .. "|r")
        else
    ...
    elseif input == "test" then
        if ct.testmode then
            EndTestMode()
            print("|cffffff00" .. L.COMBATTEXT_TEST_DISABLED .. "|r")
        else
            StartTestMode()
            print("|cffffff00" .. L.COMBATTEXT_TEST_ENABLED .. "|r")
        end
    elseif input == "reset" then
        ...
        print("|cffffff00" .. L.COMBATTEXT_TEST_USE_UNLOCK .. "|r")
        print("|cffffff00" .. L.COMBATTEXT_TEST_USE_LOCK .. "|r")
        print("|cffffff00" .. L.COMBATTEXT_TEST_USE_TEST .. "|r")
        print("|cffffff00" .. L.COMBATTEXT_TEST_USE_RESET .. "|r")
    end
end
SLASH_XCT1 = "/xct"
...
if cfg.merge_aoe_spam then
    if cfg.damage or cfg.healing then
        local pairs = pairs
        SQ = {}
        for k, v in pairs(C.aoespam.aoespam) do
...
if cfg.damage then
    ...
    if cfg.icons then
        ct.blank = C.media.texture.blank
    end
    ...
    local dmg = function()
        ...
        if (sourceGUID == ct.pguid and destGUID ~= ct.pguid) or (sourceGUID == UnitGUID("pet") and cfg.pet_damage) or (sourceFlags == gflags) then
            if eventType == "SWING_DAMAGE" then
                local amount, _, _, _, _, _, critical = select(12, CombatLogGetCurrentEventInfo())
                if amount >= cfg.treshold then
                    local rawamount = amount
                    if cfg.short_numbers == true then
                        amount = E:ShortValue(amount)
                    end
                    if critical then
                        amount = "|cffFF0000" .. cfg.crit_prefix .. "|r" .. amount .. "|cffFF0000" .. cfg.crit_postfix .. "|r"
                    end
                    if cfg.icons then
                    ...
                    if cfg.merge_aoe_spam and cfg.merge_melee then
                        ...
                        if SQ[spellId]["count"] == 1 then
                            SQ[spellId]["utime"] = time() + C.aoespam.aoespam[spellId]
                        end
            ...
            elseif eventType == "SPELL_DAMAGE" or (eventType == "SPELL_PERIODIC_DAMAGE" and cfg.dot_damage) then
            ...
            elseif eventType == "SPELL_MISSED" or eventType == "RANGE_MISSED" then
                ...
                xCT4:AddMessage(missType)
            elseif eventType == "SPELL_DISPEL" and cfg.dispel then
            ...
            elseif eventType == "SPELL_STOLEN" and cfg.dispel then
            ...
            elseif eventType == "SPELL_INTERRUPT" and cfg.interrupt then
                local id, effect = select(15, CombatLogGetCurrentEventInfo())
                local color = { 1, 0.5, 0 }
                if cfg.icons then
                    icon = GetSpellTexture(id)
            ...
            elseif eventType == "PARTY_KILL" and cfg.killingblow then
...
if cfg.healing then
    ...
    local heal = function()
        ...
        if sourceGUID == ct.pguid or sourceFlags == gflags then
            if eventType == "SPELL_HEAL" or (eventType == "SPELL_PERIODIC_HEAL" and cfg.show_hots) then
                if cfg.healing then
                    local spellId, _, _, amount, overhealing, _, critical = select(12, CombatLogGetCurrentEventInfo())
                    if C.aoespam.healfilter[spellId] then
                        return
                    end
                    if amount >= cfg.heal_treshold then
                        local color = {}
                        local rawamount = amount
                        if cfg.show_overhealing and abs(overhealing) > 0 then
                            if cfg.short_numbers == true then
                                amount = E:ShortValue(math.floor(amount - overhealing)) .. " [" .. E:ShortValue(floor(overhealing)) .. "]"
                            else
