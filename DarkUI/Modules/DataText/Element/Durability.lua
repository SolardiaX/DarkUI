...
if not C.stats or not C.stats.durability or not C.stats.durability.enable then
    return
end
...
local format, gsub, gmatch, tinsert = format, gsub, gmatch, tinsert
local print = print
...
module:Inject("Durability", {
    ...
    OnEnter = function(self)
        ...
        for slot, string in gmatch("1HEAD3SHOULDER5CHEST6WAIST7LEGS8FEET9WRIST10HANDS16MAINHAND17SECONDARYHAND", "(%d+)([^%d]+)") do
            ...
            if dur ~= dmax then
                ...
                GameTooltip:AddDoubleLine(
                    cfg.gear_icons
                            and format(
                                "|T%s:" .. t_icon .. ":" .. t_icon .. ":0:0:64:64:5:59:5:59:%d|t %s",
                                GetInventoryItemTexture("player", slot),
                                t_icon,
                                str
                            )
                        or str,
                    format("|cffaaaaaa%s/%s | %s%s%%", dur, dmax, hex, floor(perc * 100)),
                    1,
                    1,
                    1
                )
        ...
        GameTooltip:AddDoubleLine(
            " ",
            L.DATATEXT_AUTO_REPAIR .. ": " .. (C.automation.auto_repair and "|cff55ff55" .. L.DATATEXT_ON or "|cffff5555" .. L.DATATEXT_OFF),
            1,
            1,
            1,
            module.ttsubh.r,
    ...
    OnClick = function(self, button)
        if C_EquipmentSet_GetNumEquipmentSets() > 0 and button == "LeftButton" and (IsAltKeyDown() or IsShiftKeyDown()) then
            local menulist = { { text = format(gsub(EQUIPMENT_SETS, ":", ""), ""), isTitle = true, notCheckable = 1 } }
