...
if not C.stats or not C.stats.guild or not C.stats.guild.enable then
    return
end
...
local cfg = module.config.Guild

local guildTable = {}
local CURRENT_GUILD_SORTING = "class"
...
module:Inject("Guild", {
    ...
    OnClick = function(self, b)
        ...
        elseif b == "MiddleButton" and IsInGuild() then
            ...
            SortGuildRoster(
                IsShiftKeyDown() and s
                    or (
                        IsAltKeyDown() and (s == "rank" and "note" or "rank")
                        or s == "class" and "name"
                        or s == "name" and "level"
                        or s == "level" and "zone"
                        or "class"
        ...
        elseif b == "RightButton" and IsInGuild() then
            ...
            for i = 1, #guildTable do
                if (guildTable[i][7] or guildTable[i][10]) and guildTable[i][1] ~= E.myName then
                    local classc = (CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS)[guildTable[i][9]]
                    local levelc = GetQuestDifficultyColor(guildTable[i][3])
    ...
    OnEnter = function(self)
        if IsInGuild() then
            ...
            GameTooltip:AddDoubleLine(
                GetGuildInfo("player"),
                format("%s: %d/%d", GUILD_ONLINE_LABEL, online, total),
                module.tthead.r,
                module.tthead.g,
                module.tthead.b,
                module.tthead.r,
            ...
            if gmotd ~= "" then
                GameTooltip:AddLine(format("%s |cffaaaaaa- |cffffffff%s", GUILD_MOTD, gmotd), module.ttsubh.r, module.ttsubh.g, module.ttsubh.b, 1)
            end
            if cfg.maxguild ~= 0 and online >= 1 then
                ...
                for i = 1, total do
                    if cfg.maxguild and i > cfg.maxguild then
                        if online > 2 then
                            GameTooltip:AddLine(
                                format("%d %s (%s)", online - cfg.maxguild, L.DATATEXT_HIDDEN, ALT_KEY),
                                module.ttsubh.r,
                                module.ttsubh.g,
                                module.ttsubh.b
                            )
                    ...
                    if (connected or isMobile) and level >= cfg.threshold then
                        ...
                        if self.altdown then
                            GameTooltip:AddDoubleLine(
                                format("%s%s |cff999999- |cffffffff%s", grouped, name, rank),
                                zone,
                                classc.r,
                                classc.g,
                                classc.b,
                            ...
                            GameTooltip:AddDoubleLine(
                                format("|cff%02x%02x%02x%d|r %s%s%s", levelc.r * 255, levelc.g * 255, levelc.b * 255, level, name, status, grouped),
                                zone,
                                classc.r,
                                classc.g,
                                classc.b,
                ...
                GameTooltip:AddDoubleLine(
                    " ",
                    format("%s %s", L.DATATEXT_SORTING_BY, CURRENT_GUILD_SORTING),
                    1,
                    1,
                    1,
                    module.ttsubh.r,
