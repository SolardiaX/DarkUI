...
local function classColor(class, showRGB)
	...
	if not color then color = colors["PRIEST"] end

	if showRGB then
		return color.r, color.g, color.b
	else
		return "|c"..format('|cff%02x%02x%02x', color.r * 255, color.g * 255, color.b * 255)
	end
end

local function diffColor(level)
	return E:RGBToHex(GetQuestDifficultyColor(level))
end
...
local function updateGuildUI(event, addon)
	...
	E:UnregisterEvent(event, updateGuildUI)
end
E:RegisterEvent("ADDON_LOADED", updateGuildUI)
