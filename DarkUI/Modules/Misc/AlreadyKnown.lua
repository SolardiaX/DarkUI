	...
	elseif linkType == "item" then
		...
		if itemClassID == Enum.ItemClass.Battlepet and index then
			if E.isBeta then
				...
				local speciesID = E.ScanTip:SetGuildBankItem(GetCurrentGuildBankTab(), index)
			...
			if E.isBeta then
				...
				E.ScanTip:SetHyperlink(link)
				for i = 1, E.ScanTip:NumLines() do
					local text = _G["DarkUI_ScanTooltipTextLeft"..i]:GetText() or ""
					if strfind(text, COLLECTED) or text == ITEM_SPELL_KNOWN then
						knowns[link] = true
						return true
					end
