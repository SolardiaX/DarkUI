...
local FCF_GetChatWindowInfo, FCF_SetChatWindowFontSize = FCF_GetChatWindowInfo, FCF_SetChatWindowFontSize
local FCF_SavePositionAndDimensions, FCF_DockFrame = FCF_SavePositionAndDimensions, FCF_DockFrame
...
local ChatFrameToggleVoiceMuteButton = ChatFrameToggleVoiceMuteButton
...
if cfg.auto_width then
    ...
    cfg.width = ((E.screenWidth - (holder and holder:GetWidth() or 1024)) / 2 - 40) * renderScale
end
...
local function SetupChat()
    for i = 1, NUM_CHAT_WINDOWS do
        ...
        if frame.skinned ~= true then SetChatStyle(frame) end
    end

    -- Remember last channel
    local var
    if cfg.sticky == true then
        var = 1
    else
        var = 0
    end
